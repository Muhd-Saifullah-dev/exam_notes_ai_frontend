...
const markDownComponent = {
  ...
  h2: ({ children }) => (
    <h2 className="text-xl font-semiibold text-indigo-600 mt-5 mb-3">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
  ...
  ul: ({ children }) => (
    <ul className="  text-gray-700 list-disc ml-6 space-y-1">{children}</ul>
  ),
    ...
    <div className="mt-6 p-3 space-y-10 bg-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button className="px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700" onClick={()=>downloadPdf(result)}>
            ⬇️ Download PDF
          </button>
        </div>
      </div>
